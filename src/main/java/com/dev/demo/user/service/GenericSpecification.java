...
public class GenericSpecification<T> {
    ...
    public static <T> Specification<T> build(List<String> filterStrings, String search) {
        return (root, query, criteriaBuilder) -> {
            List<Predicate> predicates = new ArrayList<>();

            List<String> searchOperators = List.of("endsWith");
            if (FilterUtils.isValid(search) && FilterUtils.isValid(searchOperators)) {
                applySearch(root, criteriaBuilder, search, searchOperators, predicates);
            }
    ...
    private static <T> void applySearch(Root<T> root, CriteriaBuilder criteriaBuilder, String search, List<String> searchOperators, List<Predicate> predicates) {
        root.getModel().getSingularAttributes().stream()
                .filter(attribute -> attribute.getJavaType() == String.class)
                .forEach(attribute -> {
                    Path<String> fieldPath = root.get(attribute.getName());
                    predicates.add(criteriaBuilder.or(FilterUtils.getSearchPredicates(criteriaBuilder, fieldPath, search, searchOperators)));
                });
    }
